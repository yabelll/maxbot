...
bot.action('startEp3', async ctx => {
  try {
    ...
    await delay(5000);
    await ctx.reply(
      `<b>Тюленюся, ее голос вновь обретает энергичный и обнадёживающий тон:</b> Вы увидели проблему во всей её сложности. Не падайте духом. Знание — это сила. 
...
<b>Давайте составим ваш личный план спасения мира, уровень за уровнем.</b>`,
    ...
    await delay(5000);
    await ctx.reply(
      `<b>Выбор А:</b> «Осознанное потребление»
Откажитесь от одноразового. Многоразовая кружка, бутылка, сумка-шопер — это не просто тренд, это ваш личный вклад в сокращение «УДУШЬЯ».

<b>Выбор Б:</b> «Энергия в ваших руках»
Выключайте свет, не оставляйте гаджеты на зарядке на ночь, выбирайте энергоэффективные приборы. Это ваш удар по «ЛИХОРАДКЕ».
...
Организуйте в своей школе или районе бесплатную ярмарку или книгообмен. Это снижает потребление и отходы.

<b>Выбор Б:</b> «Точка сбора»
Инициируйте установку контейнеров для батареек и опасных отходов в вашей школе. Одна выброшенная батарейка отравляет 20 квадратных метров земли.
...
bot.action('choise3_2', async ctx => {
  try {
    ...
    await delay(5000);
    await ctx.reply(
      `<b>Выбор А:</b> «Профессия со смыслом»
Используйте свои навыки — программирование, дизайн, маркетинг — для помощи 'зелёным' стартапам.

<b>Выбор Б:</b> «Станьте голосом»
Рассказывайте о своём пути в соцсетях. Ваша искренняя история вдохновит больше людей, чем сухие отчёты.
...
bot.action('startEpilog', async ctx => {
  try {
    ...
    await delay(5000);
    await ctx.reply(
      `<b>Тюленюся:</b> Миссия в будущем завершена. Но ваша миссия в настоящем только началась. 
      
Основываясь на ваших действиях, я подготовила для вас персональный план по спасению мира:
